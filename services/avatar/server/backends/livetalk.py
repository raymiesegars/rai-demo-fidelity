"""LiveTalk backend — warm worker → stream full-frame generative video.

GAIR-NLP LiveTalk: distilled causal Wan 1.3B (4-step). Official target is Linux;
Windows uses SDPA (no flash-attn). Clip/utterance path (public code is not
true WebRTC streaming yet).

Requires: .\\setup_livetalk.ps1
Run:      .\\run_server.ps1 -Backend livetalk
"""
...
logger = logging.getLogger("avatar.livetalk")


class LiveTalkWorker:
    ...
    def start(self) -> None:
        with self._lock:
            if self._proc and self._proc.poll() is None:
                return
            env = os.environ.copy()
            env["LIVETALK_ROOT"] = str(self.vendor)
            env["PYTHONUNBUFFERED"] = "1"
            env.setdefault(
                "LIVETALK_PROMPT",
                "A realistic video of a person speaking directly to the camera "
                "with natural expressions and precise lip sync.",
            )
            try:
            ...
            logger.info("Starting LiveTalk worker (warm)…")
            ...
            threading.Thread(target=self._drain_stderr, daemon=True, name="livetalk-stderr").start()
            ready = self._read_locked(timeout_s=600.0)
            if not ready or not ready.get("ok"):
                err = (ready or {}).get("error", "no ready message")
                self.stop()
                raise RuntimeError(f"LiveTalk worker failed to start: {err}")
            logger.info("LiveTalk worker ready in %ss", ready.get("load_s"))
    ...
    def _drain_stderr(self) -> None:
        ...
        for line in proc.stderr:
            line = line.rstrip()
            if line:
                logger.info("livetalk-worker: %s", line[:500])
    ...
    def _read_locked(self, timeout_s: float) -> dict | None:
        ...
        if t.is_alive():
            raise TimeoutError(f"LiveTalk worker timed out after {timeout_s:.0f}s")
        if "err" in box:
            raise box["err"]
        line = box.get("line") or ""
        if not line:
            raise RuntimeError("LiveTalk worker exited (no response)")
    ...
    def _request(self, req: dict, timeout_s: float = 900.0) -> dict:
        ...
        if not resp or not resp.get("ok"):
            ...
            raise RuntimeError(f"LiveTalk {req.get('cmd')} failed: {err}\n{trace[-1500:]}")
        return resp
    ...
    def infer(self, audio_path: Path, output_path: Path) -> Path:
        ...
        logger.info("LiveTalk infer %dms → %s", resp.get("ms"), Path(resp["video_path"]).name)
        return Path(resp["video_path"])


class LiveTalkBackend(UtteranceStreamEngine):
    backend_id = "livetalk"
    backend_name = "LiveTalk"

    def __init__(self, root: Path) -> None:
        super().__init__(chunk_frames=12, fps=16)
        self.root = root
        self.vendor = Path(os.environ.get("LIVETALK_ROOT", root / "vendor" / "LiveTalk"))
        self.python = Path(os.environ.get(
            "LIVETALK_PYTHON",
            root / ".venv_livetalk" / "Scripts" / "python.exe",
        ))
        if not self.python.is_file():
            self.python = Path(sys.executable)

        worker_py = root / "scripts" / "livetalk_worker.py"
        dit = self.vendor / "pretrained_checkpoints" / "LiveTalk-1.3B-V0.1"
        if not self.vendor.is_dir() or not worker_py.is_file():
            raise RuntimeError(
                f"LiveTalk incomplete.\n  vendor: {self.vendor}\n  worker: {worker_py}\n"
                "Run: .\\setup_livetalk.ps1"
            )
        if not dit.is_dir():
            raise RuntimeError(
                f"LiveTalk checkpoint missing: {dit}\nRun: .\\setup_livetalk.ps1"
            )

        self._work = root / "uploads" / "_livetalk_work"
        self._work.mkdir(parents=True, exist_ok=True)
        self._avatar_img: Path | None = None
        self._worker = LiveTalkWorker(self.python, worker_py, self.vendor)
    ...
    def render_utterance(self, audio_f32_16k: np.ndarray) -> list[np.ndarray]:
        ...
        try:
            ...
            pass
        if not frames:
            raise RuntimeError("LiveTalk produced no frames")
