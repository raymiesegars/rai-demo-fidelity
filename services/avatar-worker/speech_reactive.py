...
def reactive_mouth_rect(box: list[int]) -> tuple[int, int, int, int]:
    """Tight crop on the lips only — not the nose or full jaw."""
    y1, y2, x1, x2 = box
    fh, fw = y2 - y1, x2 - x1
...
class SpeechReactiveMouth:
    """Subtle lip-only motion synced to agent speech."""

    def __init__(self) -> None:
        self._openness = 0.0
        self._attack = float(os.environ.get("MOUTH_ATTACK", "0.55"))
        self._decay = float(os.environ.get("MOUTH_DECAY", "0.22"))
        self._max_stretch = float(os.environ.get("MOUTH_MAX_STRETCH", "0.14"))
    ...
    def apply(self, frame: np.ndarray, box: list[int] | None) -> np.ndarray:
        ...
        # Split upper / lower lip; only the lower lip moves down slightly.
        ...
        gap_px = max(0, int(h * o * self._gap * 0.12))
        ...
        # Small inner-mouth shadow (lips only, not whole face).
        if o > 0.15:
            cx, cy = w // 2, seam + max(1, gap_px // 2)
            ax = max(1, int(w * 0.10 * o))
            ay = max(1, int(h * 0.06 * o))
            shadow = np.zeros((h, w), dtype=np.float32)
            cv2.ellipse(shadow, (cx, cy), (ax, ay), 0, 0, 360, 1.0, -1)
            shadow = cv2.GaussianBlur(shadow, (5, 5), 0) * o * 0.25
