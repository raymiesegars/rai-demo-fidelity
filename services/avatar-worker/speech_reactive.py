...
def _warp_mouth_open(
    ...
    """Smooth lip-band warp — no hard seam split, motion peaks on lips not philtrum/chin."""
    h, w = roi.shape[:2]
    ...
    upper_y = h * 0.32
    lower_y = h * 0.58
    upper_band = _lip_band(yy, upper_y, h * 0.11)
    lower_band = _lip_band(yy, lower_y, h * 0.12)
